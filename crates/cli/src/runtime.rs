...
use crate::output::{
    ConfiguredCommandOutcome, print_json_check_success_result, print_json_compile_success_result,
    print_json_failure_result, print_success_summary,
};
    ...
    match loader.load().and_then(|config| {
        ...
        Ok(outcome) => {
            ...
            if command.format() == OutputFormat::Json {
                match &outcome {
                    ConfiguredCommandOutcome::Check(check_outcome) => {
                    ...
                    ConfiguredCommandOutcome::Compile(compile_outcome) => {
                        print_json_compile_success_result(
                            command,
                            config_path.as_deref(),
                            compile_outcome,
                            &diagnostics,
